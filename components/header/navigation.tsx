...
const SocialIcon = ({
  ...
  const iconClass = `${className} transition-all duration-300 hover:scale-110`;
    ...
    <nav className="flex">
      <AnimatePresence>
        {canFade && (
          ...
          >
            {items.map(({ title, url, type }, index) => (
              <motion.li
                key={index}
                variants={item}
                ...
                >
                  {type === "logo" ? (
                    <SocialIcon
                      ...
                      className="w-5 h-5 lg:w-6 lg:h-6 opacity-80 hover:opacity-100"
                    />
                  ) : (
                    title
                  )}
